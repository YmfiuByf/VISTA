...
input_path = r"D:\pycharmProject\paper\videos\final_street\street1_resized.avi"
...
def main():
    ...
    parser.add_argument('--save_folder', default='D:\\pycharmProject\\video semantic communication\\test_results_demo', type=str)
    ...
    with open("D:\\pycharmProject\\video semantic communication\\k_list.txt", "r", encoding="utf-8") as f:
    ...
    out = cv2.VideoWriter(output_path, fourcc, fps, size_out)
    num = int(fps * clip.duration)
    success, frame = video.read()
    frame = cv2.resize(frame,(640, 360), Image.ANTIALIAS)
    print(type(frame))
    back = frame
    out.write(frame)
    _,img1 = video.read()
    _,img2 = video.read()
    img3 = cv2.imread(r"D:\pycharmProject\paper\videos\final_street\street1_resized_beh1_JSCC_SNR=-3.jpg")
    img4 = cv2.imread(r"D:\pycharmProject\paper\videos\final_street\street1_resized_beh1_JSCC_SNR=-6.jpg")
    flow = get_flow(img1,img2)
    img0 = one_interpolation(img3,img4)*255
    cv2.imwrite(r"D:\pycharmProject\paper\videos\final_street\test_env.jpg",img0)
    # for i in tqdm(range(num)):
    #     success, frame = video.read()
    #     frame = cv2.resize(frame,(640, 360), Image.ANTIALIAS)
    #     front = back
    #     back = frame
    #     k = int(float(k_list[i+1]))
